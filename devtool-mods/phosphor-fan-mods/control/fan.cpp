...
{
    log<level::INFO>("Fan initializing fan");
    std::string path;
    log<level::INFO>(("Fan Name: " + std::get<fanNamePos>(def)  + ", target interface: " +
     std::get<targetInterfacePos>(def) + ", target control path: " + std::get<targetControlPathPos>(def)).c_str());
    auto sensors = std::get<sensorListPos>(def);
    for (auto& s : sensors)
    {
        path = std::get<targetControlPathPos>(def) + s;
        log<level::INFO>(("Fan attempting to get service for the sensors at path: " + path).c_str());
    ...
    if (!path.empty())
    {
        log<level::INFO>("Fan found the path, now trying to get the property");
