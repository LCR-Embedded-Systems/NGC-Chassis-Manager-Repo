...
GetSensorResponse readingData(const Info& sensorInfo)
{
    ...
    log<level::INFO>("INFO: readingData enter");
    ...
    log<level::INFO>(("INFO: readingData sensor interface: " + sensorInfo.sensorInterface).c_str());
    log<level::INFO>(("INFO: readingData sensor path: " + sensorInfo.sensorPath).c_str());

    auto service = ipmi::getService(bus, sensorInfo.sensorInterface,
                                    sensorInfo.sensorPath);

    log<level::INFO>("INFO: readingData service acquired");
...
#endif
    log<level::INFO>(("INFO: readingData acquiring propValue with params: sensorPath = " + sensorInfo.sensorPath + 
    " first " + sensorInfo.propertyInterfaces.begin()->first + " second " + sensorInfo.propertyInterfaces.begin()->second.begin()->first).c_str());
    auto propValue = ipmi::getDbusProperty(
        bus, service, sensorInfo.sensorPath,
        sensorInfo.propertyInterfaces.begin()->first,
        sensorInfo.propertyInterfaces.begin()->second.begin()->first);
    log<level::INFO>("INFO: readingData propValue acquired");
    ...
    log<level::INFO>("INFO: readingData returning now");
