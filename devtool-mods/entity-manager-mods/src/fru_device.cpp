...
static void makeProbeInterface(size_t bus, size_t address,
                               sdbusplus::asio::object_server& objServer)
{
    std::cerr << "makeProbeInterface enter\n";
    if (isMuxBus(bus))
    {
...
{
    std::cerr << "i2cSmbusWriteThenRead enter with address " << address ;
    ...
    std::cerr << "i2cSmbusWriteThenRead read and write finish\n";
                             ...
                             off_t offset, size_t len, uint8_t* buf)
{
    if (!is16bit)
    {
        std::cerr << "readBlockData block data is not 16 bit\n";
        return i2c_smbus_read_i2c_block_data(file, static_cast<uint8_t>(offset),
                                             len, buf);
    }
    std::cerr << "readBlockData block data IS 16 bit\n";
               ...
               sdbusplus::asio::object_server& objServer)
{
    ...
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            if (device.empty())
            {
                std::cerr << "getBusFrus Device empty\n";
                continue;
            }
            std::cerr << "getBusFrus emplacing device into devices map\n";
...
void addFruObjectToDbus(
    ...
    std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    std::cerr << "addFruObjectToDbus enter\n";
