...
{
    log<level::INFO>("Zone Initializing zone now");
    auto& fanDefs = std::get<fanListPos>(def);
    log<level::INFO>("Zone acquired fandefs");
    for (auto& fanDef : fanDefs)
    {
        log<level::INFO>("Zone making fans for each fandef");
    ...
    {
        log<level::INFO>("Zone in mode control");
        // Process any zone handlers defined
        for (auto& hand : std::get<handlerPos>(def))
        {
            log<level::INFO>("Zone handlerPos for each def");
            hand(*this);
        }
...
{
    log<level::INFO>("Zone setSpeed called");
    if (_isActive)
    {
        _targetSpeed = speed;
        for (auto& fan : _fans)
...
{
    log<level::INFO>("Zone setFullSpeed called");
    if (_fullSpeed != 0)
    {
...
{
    log<level::INFO>("Zone setActiveAllow called");
...
{
    log<level::INFO>("Zone removeService called");
    try
    {
...
void Zone::setServiceOwner(const Group* group, const std::string& name,
                           const bool hasOwner)
{
    log<level::INFO>("Zone setServiceOwner called");
    try
    {
        auto& sNames = _services.at(*group);
        auto it = std::find_if(sNames.begin(), sNames.end(),
...
{
    log<level::INFO>("Zone setServices called");
...
{
    log<level::INFO>("Zone setFloor called");
...
{
    log<level::INFO>("Zone requestSpeedIncrease called");
...
{
    log<level::INFO>("Zone requestSpeedDecrease called");
...
{
    log<level::INFO>("Zone initEvent called");
...
{
    log<level::INFO>("Zone removeEvent called");
...
{
    log<level::INFO>("Zone findTimer called");
...
{
    log<level::INFO>("Zone addTimer called");
...
void Zone::timerExpired(const Group& eventGroup,
                        const std::vector<Action>& eventActions)
{
    log<level::INFO>("Zone timerExpired called");
    // Perform the actions
    std::for_each(
...
{
    log<level::INFO>("Zone handleEvent called");
    // Handle the callback
...
{
    log<level::INFO>("Zone getService called");
...
{
    log<level::INFO>("Zone addServices called");
...
{
    log<level::INFO>("Zone getPersisted called");
...
{
    log<level::INFO>("Zone current called");
...
{
    log<level::INFO>("Zone saveCurrentMode called");
...
{
    log<level::INFO>("Zone restoreCurrentMode called");
    auto current = ThermalObject::current();
    fs::path path{CONTROL_PERSIST_ROOT_PATH};
    path /= std::to_string(_zoneNum);
    path /= "CurrentMode";
