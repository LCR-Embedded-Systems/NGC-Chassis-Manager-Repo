                   ...
                   off_t& baseOffset)
{
    ...
    std::cerr << "findFRUHeader exit, returning false\n";
    return false;
}
...
{
    std::cerr << "readFRUContents enter\n";
    std::array<uint8_t, I2C_SMBUS_BLOCK_MAX> blockData{};
    off_t baseOffset = 0x0;

    if (!findFRUHeader(reader, errorHelp, blockData, baseOffset))
    {
        std::cerr << "readFRUContents no headers, returning\n";
        return {};
    }
